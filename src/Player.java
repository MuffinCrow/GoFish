...
public class Player {
    ...
    public Card getCardByNeed() {
        ...
        for (int i = 0; i < hand.size() - 1; i++) {
            ...
            if (count > frequency && !done.contains(hand.get(i).getRank())) {    // updates which card is the most frequently occurring
                index = i;
                frequency = count;
                result = i;
            }
        }
        done.add(hand.get(result).getRank());
        double randomNum = 0;
        randomNum = Math.random()*5;
        if (done.size() == Card.getUniqueRanks(this.hand) || randomNum > 1) {
            done = new ArrayList<>();
        }
